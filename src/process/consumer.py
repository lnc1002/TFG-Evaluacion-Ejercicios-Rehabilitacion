...
from extraOpt import ang as ang_
import pathlib
#Logging
import logging
...
def op(package):
    ...
    if img is not None and k is not None:
        try:
            ...
            with open("angulos2.txt","a") as f:
                f.writelines(str(a))
            with open("posiciones2.txt","a") as f:
                f.writelines(str(k))
        except:
            traceback.print_exc()
            lg.debug(traceback.format_exc())
    ...
    try:
        if SAVE:
            ...
            with open("test2.txt", "w+") as f:
                f.writelines("Esta es la imagen: ")
    except:
        traceback.print_exc()
        lg.debug(traceback.format_exc())
...
#anonimizeStream.map(lambda x: an(x))
